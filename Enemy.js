class Enemy {
  ...
  update() {
    ...
    if (this.direction.y > 0) tileSiMovementY = map[Math.floor(this.py + 0.9)][Math.floor(sauvegardeX)];
		...
		if (Math.floor(this.nextTile.x) == Math.floor(this.px + offset) && Math.floor(this.nextTile.y) == Math.floor(this.py  + offset) && Math.floor(this.px  + offset) != 0 && Math.round(this.px  + offset) != map.length) 
